    ...
    try {
      ...
      if (data.type === 'terminal_input') {
        ...
        exec(data.command, { cwd: process.cwd() }, (err, stdout, stderr) => {
          ...
          // Notify mobile device that command execution finished
          ws.send(JSON.stringify({
            type: 'run_complete',
            title: '⚡ Command Finished',
            message: `Command "${data.command.slice(0, 30)}" finished with code ${err ? (err.code || 1) : 0}`
          }));
        });
        return;
      }
      ...
      if (data.type === 'user_prompt') {
        ...
        setTimeout(() => {
          ws.send(JSON.stringify({
            type: 'run_complete',
            title: '⚡ Antigravity Run Complete',
            message: `Execution of "${data.content.slice(0, 40)}" completed.`
          }));
        }, 1500);
      }
