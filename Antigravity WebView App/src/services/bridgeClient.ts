...
class BridgeClientService {
  ...
  private handleBridgeMessage(data: any) {
    switch (data.type) {
      ...
      case 'agent_finished':
      case 'notification':
        dispatchNotification(data.title || '⚡ Antigravity Run Complete', data.body || data.message || 'Task finished successfully.');
        break;
      case 'tool_approval_request':
        this.pendingApprovals.push(data.toolCall);
        this.notifyApprovals();
        dispatchNotification('⚠️ Approval Required', `Action waiting for review: ${data.toolCall?.summary || data.toolCall?.name || 'Pending approval'}`);
        break;
  ...
  private simulateAgentResponse(prompt: string, settings?: AgentSettings) {
    ...
    const interval = setInterval(() => {
      if (charIndex < responseText.length) {
        ...
        const summaryText = prompt.length > 40 ? prompt.slice(0, 40) + '...' : prompt;
        dispatchNotification('⚡ Antigravity Run Complete', `Completed: "${summaryText}"`);
        ...
        if (prompt.toLowerCase().includes('deploy') || prompt.toLowerCase().includes('run')) {
          ...
          this.pendingApprovals.push(approvalItem);
          this.notifyApprovals();
          dispatchNotification('⚠️ Approval Required', `Review command execution: ${prompt.slice(0, 30)}`);
        }
      }
    }, 25);
  }
